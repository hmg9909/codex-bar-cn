...
struct CodexProviderImplementation: ProviderImplementation {
    ...
    func settingsToggles(context: ProviderSettingsContext) -> [ProviderSettingsToggleDescriptor] {
        ...
        return [
            ProviderSettingsToggleDescriptor(
                ...
                title: "历史跟踪",
                subtitle: "保存本地 Codex 使用历史（8 周），用于个性化预测用尽时间。",
            ...
            ProviderSettingsToggleDescriptor(
                ...
                subtitle: [
                    "可选。",
                    "开启后通过 chatgpt.com 显示代码审查额度、用量明细和余额历史。",
                ].joined(separator: " "),
    ...
    func appendUsageMenuEntries(context: ProviderMenuUsageContext, entries: inout [ProviderMenuEntry]) {
        ...
        if let credits = context.store.credits {
            entries.append(.text("余额: \(UsageFormatter.creditsString(from: credits.remaining))", .primary))
